...
void main() async {
  ...
  runApp(MaterialApp(
    ...
    home:FakeLoginScreen(),// 根據檢查結果設置初始畫面
  ));
}
...
Future<Widget> _getInitialScreen() async {
  try {
    ...
    if (userProfile != null) {
      ...
      // 直接進入 OnboardingScreen，讓使用者輸入姓名開始
...
final GoogleSignIn _googleSignIn = GoogleSignIn(
  ...
  clientId: '599090774181-db855virbaurhemr46sk84b2ih5nf6ju.apps.googleusercontent.com',
);
...
Future<void> _pickImage(ImageSource source) async {
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("拍照失敗，請檢查相機權限"))
    );
  }
}
  ...
  onPressed: () async {
    ...
    setState(() {}); // 重新渲染畫面
                  ...
                  TextFormField(
                    ...
                    validator: (val) {
                      if (val == null || val.isEmpty) return '請輸入熱量';
                      final n = int.tryParse(val);
                      if (n == null) return '請輸入數字';
                      if (n < 1 || n > 5000) return '請輸入合理範圍（1-5000）';
                      return null;
                    },
                  ),
                  SizedBox(height: 16),
...
Future<void> _pickImage(ImageSource source) async {
  try {
    ...
    // 🚀 如果是 iOS 模擬器，並且來源是相機，則自動改用相簿
    if (Platform.isIOS && source == ImageSource.camera) {
      print("iOS 模擬器不支援相機，自動改用相簿");
    ...
    print("Error picking image: $e");
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("拍照或選擇圖片失敗，請檢查權限")),
    );
  }
}
